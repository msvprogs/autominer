...
namespace Msv.AutoMiner.Rig.System.Unix
{
    public class UnixSystemStateProvider : ISystemStateProvider
    {
        ...
        {
            var osInfo = ParseKeyValuePairs(M_FileReader.ReadLines(DistribInfoPath), "=")
        ...
        {
            return SplitBy(M_FileReader.ReadLines(CpuInfoPath), string.Empty)
            ...
            int GetFrequencyValue(string infoPath)
            {
                ...
                return (int)cpuInfoDirectory.GetDirectories("cpu?")
                    ...
                    .Select(y => M_FileReader.ReadLines(Path.Combine(y.FullName, infoPath)).FirstOrDefault())
        ...
        {
            var keyValuePair = ParseKeyValuePairs(M_FileReader.ReadLines(MemInfoPath))
