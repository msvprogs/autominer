...
namespace Msv.AutoMiner.Service.Video.NVidia
{
    public class NVidiaVideoSystemStateProvider : IVideoSystemStateProvider
    {
        ...
        private static VideoSystemState BuildStateInfo()
        {
            ...
            int deviceCount;
            if (!ThrowOnFatalError(NativeNvml.DeviceGetCount(out deviceCount)))
            {
            ...
            {
                IntPtr handle;
                if (!ThrowOnFatalError(NativeNvml.DeviceGetHandleByIndex(i, out handle)))
                    return state;
                ...
                uint returnedValue;
                if (ThrowOnFatalError(NativeNvml.DeviceGetClockInfo(handle, NvmlClockType.Sm, out returnedValue)))
